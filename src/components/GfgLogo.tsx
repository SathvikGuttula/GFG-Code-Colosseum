...
interface GfgLogoProps {
  className?: string;
  size?: 'sm' | 'md' | 'lg' | 'xl';
  showText?: boolean;
}

export const GfgLogo: React.FC<GfgLogoProps> = ({
  className = '',
  size = 'md',
  showText = true
}) => {
  ...
  return (
    <div className={`flex items-center gap-2.5 select-none ${className}`}>
      {/* Official GeeksforGeeks Emblem */}
      <div className="relative flex items-center justify-center">
        <svg
          viewBox="0 0 76.533 39.026"
          className={`${sizeMap[size]} w-auto drop-shadow-xs`}
          fill="none"
      ...
      {showText && (
        <div className="flex items-center leading-none tracking-tight font-extrabold text-lg sm:text-xl font-mono">
          <span className="text-[#2F8D46] dark:text-[#38b757]">Geeks</span>
          <span className="text-zinc-800 dark:text-zinc-100">for</span>
          <span className="text-[#2F8D46] dark:text-[#38b757]">Geeks</span>
        </div>
      )}
    </div>
